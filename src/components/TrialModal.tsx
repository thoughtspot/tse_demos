// Upgrade prompt shown on the 3rd Ask SalesSpot question.
      ...
      <div className="sl-trial-modal" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
        ...
        {done ? (
          <div className="sl-trial-body" style={{ textAlign: 'center' }}>
            <div className="sl-trial-icon" style={{ margin: '0 auto' }}><Check size={24} /></div>
            <h2 className="sl-trial-title" style={{ marginTop: 18 }}>
              {done === 'paid' ? 'Query pack added' : 'Meeting scheduled'}
            </h2>
            <p className="sl-trial-sub">
              {done === 'paid'
                ? <>Your counter has been reset to <strong>500 queries</strong>. You can keep asking SalesSpot AI right away.</>
          ...
          <div className="sl-trial-body">
            ...
            <p className="sl-trial-fine">
              Demo experience — no payment is taken. Talk to your SalesSpot account team for production pricing.
            </p>
          </div>
        ) : (
          <div className="sl-trial-body">
            ...
            <div className="plg-actions">
              ...
              <button className="plg-btn pri" disabled={!selDay || !selSlot} onClick={() => setDone('booked')}>
              ...
              {selDay && (
                <>
                  ...
                  <div className="plg-slots">
                    {SLOTS.map((s) => (
                      <button key={s} className={`plg-slot ${selSlot === s ? 'on' : ''}`} onClick={() => setSelSlot(s)}>{s}</button>
                    ))}
                  </div>
                </>
              )}
