// Ask HP — the custom HP AI experience (moved here from the
// Analytics "HP AI" modal). Left: the Spotter answer canvas with a
// branded empty-state landing (logo + worksheet sample questions) and its own
// input bar hidden. Right: a "HP AI" expert pane that tracks the
// question history and takes follow-ups.
//
// Each question is routed (via routeMessage): general HP questions
...
export default function AskHP() {
  ...
  const runSpotterQuery = useCallback(
    (query: string) => {
      try {
        ...
        console.warn('[hp-ai] SpotterSearch failed:', e);
  ...
  async function send(preset?: string) {
    ...
    const result: RouteResult = preset
      ...
      if (result.kind === 'analytics') {
        ...
        // General HP question → answered from the knowledge base; Spotter
  ...
  const remaining = Math.max(0, HP_TRIAL_QUESTIONS - askedCount);
      ...
      <div className="sl-ai-page">
          ...
          <div className={`sl-ai-canvas${spotterActive ? ' is-spotter-active' : ''}`}>
            {!currentQuery && (
              <div className="sl-ai-empty">
                <HPLogo className="sl-ai-empty-logo" size={58} wordmark={false} />
                <span className="sl-ai-empty-eyebrow">
                  <Sparkles size={14} /> Powered by HP AI
                </span>
        ...
        <div className="sl-ai-pane">
          ...
          <div className="sl-ai-pane-footer">
            <a href="https://www.hp.com/legal/privacy-notice" target="_blank" rel="noopener noreferrer">
              Privacy Policy
            </a>
            <span>Powered by HP</span>
          </div>
        </div>
      </div>
