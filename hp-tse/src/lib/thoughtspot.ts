...
/**
 * Initialise the ThoughtSpot Visual Embed SDK with HP styling.
 * Styling (CSS variables + spotter icon) comes straight from hp_fin.json
...
export function initThoughtSpot(username: string, password: string) {
  ...
  init({
    ...
    customizations: {
      ...
      // Global text overrides — replaces "Spotter" -> "HP AI" (and the
...
/**
 * Customizations for the Analytics liveboard — the active-theme HP
 ...
 *
 * The viz action buttons (bell / "HP AI" / …) read their hover bg from
...
export function liveboardCustomizations(theme: ThemeName) {
  const hoverBg = theme === 'dark' ? '#152535' : '#d6eefa';
  const activeBg = theme === 'dark' ? '#0f1e2d' : '#c0e4f8';
  ...
  const tabGreen = theme === 'dark' ? '#0096D6' : '#0077B5';
  ...
  const APP_FONT = "'Questrial', 'Inter', system-ui, -apple-system, 'Segoe UI', sans-serif";
 ...
 * "My Reports" listing in hp_fin.json). Sorted most-recently-modified
...
export async function fetchCadences(
  ...
  // when filtered to HP_ORG_ID. If none work, fall back to the unfiltered
  // list rather than showing nothing.
  let orgClause = '';
  let orgColumn = '';
  for (const orgCol of ORG_COLUMN_CANDIDATES) {
    const r = await searchData(`[${cadenceCol}] [${orgCol}] = '${HP_ORG_ID}'`);
    if (r && r.data_rows?.length) {
      orgClause = ` [${orgCol}] = '${HP_ORG_ID}'`;
      orgColumn = orgCol;
  ...
  if (!orgClause) {
    console.warn(
      `[hp] No org column matched ${HP_ORG_ID} (tried: ${ORG_COLUMN_CANDIDATES.join(
        ', ',
