...
// The date-filter pill names the column it filters. Default it from
// columns.date so an unset label is accurate rather than a leftover guess.
if (spec.content && spec.content.filters && !spec.content.filters.dateLabel) {
  spec.content.filters.dateLabel = (spec.columns && spec.columns.date) || 'Date';
}
const contentFile =
