...
| `columns` | Data / inline / filters | `inlineName[]` (candidate list), `inlineMetrics[]` (`{label,format,candidates[]}`, ≤3), `filterPrimary[]`, `filterSecondary[]`, `date`, `hierarchy{segment,rep,cadence}`. `date` also names the date-filter pill — it defaults `content.filters.dateLabel`, so set that field to override the wording (e.g. `Claim activity` for a `Claim Activity Date` column). Candidate lists = first that returns data wins. |
